...
def summary_value(n_intervals, select_trees, select_random_state):
    ...
    weather_data.drop(['Date', 'Time', 'Direction', 'Visibility (km)',
                       ...
                       'Temp (°C)', 'Wind (km/h)', 'RealFeelTemp (°C)'], axis = 1, inplace = True)

    df1 = pd.concat([daily_hourly_values, weather_data], axis = 1)
    df1.drop(['Date', 'Hour'], axis = 1, inplace = True)
    df1.loc[df1['SolarIrradiance (W/m2)'] == 0, ['DewPoint (°C)', 'Hum (%)', 'UVIndex', 'CloudCover (%)']] = 0
    ...
    filter_weather_yes_values = weather_data1[
        ...
        ['SolarIrradiance (W/m2)', 'DewPoint (°C)', 'Hum (%)', 'UVIndex', 'CloudCover (%)']]
    yes_df1 = pd.concat([yes_hourly_values, filter_weather_yes_values], axis = 1)
    yes_df1.drop(['Date', 'Hour'], axis = 1, inplace = True)
    yes_df1.loc[yes_df1['SolarIrradiance (W/m2)'] == 0, ['DewPoint (°C)', 'Hum (%)', 'UVIndex', 'CloudCover (%)']] = 0
    yes_count_total_rows = len(yes_df1)
    yes_independent_columns = yes_df1[['SolarIrradiance (W/m2)', 'DewPoint (°C)', 'Hum (%)', 'UVIndex', 'CloudCover (%)']][
    ...
    forcasted_yes_values.loc[
        forcasted_yes_values['SolarIrradiance (W/m2)'] == 0, ['DewPoint (°C)', 'Hum (%)', 'UVIndex', 'CloudCover (%)']] = 0
    ...
    mv_pe = df3['Power (KW)'].sum()
    mv_mse = metrics.mean_squared_error(last_day_hourly_values['Power (KW)'], df3['Power (KW)'])
    mv_rmse = np.sqrt(mv_mse)
    mv_mae = metrics.mean_absolute_error(last_day_hourly_values['Power (KW)'], df3['Power (KW)'])
    mv_rs = metrics.r2_score(last_day_hourly_values['Power (KW)'], df3['Power (KW)'])
    ...
    if time_name >= '00:00:00' and time_name <= '11:59:59':
        ...
        independent_columns = df1[['SolarIrradiance (W/m2)', 'DewPoint (°C)', 'Hum (%)', 'UVIndex', 'CloudCover (%)']][
                              0:count_total_rows]
        ...
        forcasted_data = df1[['SolarIrradiance (W/m2)', 'DewPoint (°C)', 'Hum (%)', 'UVIndex', 'CloudCover (%)']].tail(12)
        ...
        df4 = pd.read_csv('xgboost model/today_predicted_chart_data.csv')
        data_12 = df4['Power (KW)'].sum()
        ...
        forcasted_data = df1[['SolarIrradiance (W/m2)', 'DewPoint (°C)', 'Hum (%)', 'UVIndex', 'CloudCover (%)']].tail(24)
